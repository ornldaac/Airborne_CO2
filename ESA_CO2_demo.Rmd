...
ggplot(pdat, aes(x=time.lab,y=alt)) + geom_point(aes(color=co2)) +
	...
	labs(x="Time (UTC)", y="Altitude (m)", caption="Figure 4.  CO2 concentrations by altitude.") +
...
tm_shape(map) + tm_rgb() + tm_shape(spl) + 
  ...
  tm_credits("Figure 5. Northeast portion of flight path from Fig. 3.", align="left", size=1, width=1) + 
...
ggplot(pdat, aes(x=time.lab,y=alt)) + geom_point(aes(color=co2)) +
	...
	scale_x_datetime(breaks = scales::date_breaks("5 mins"), 
	 	date_labels = "%H:%M") +	
	labs(x="Time (UTC)", y="Altitude (m)", caption="Figure 6.  CO2 concentrations by altitude.") +
  theme(legend.position="bottom", plot.caption=element_text(hjust=0, size=12))
```
...
tm  + tm_add_legend("line", col=r.colr, labels=flt, title="Flight IDs") + 
  ...
  tm_credits("Figure 7. Path of flights from southeast Alaska.", align="left", size=1) + 
  tm_layout(attr.outside = T, attr.position=c("left", "bottom"), attr.outside.position = "bottom", asp=1.05,
...
ggplot(pdat, aes(x=time.lab,y=alt)) + geom_point(aes(color=co2)) +
	...
	scale_x_datetime(breaks = scales::date_breaks("15 days"), 
	 	date_labels = "%m-%d") +
	labs(x="Time (UTC)", y="Altitude (m)", caption="Figure 8.  CO2 concentrations by altitude for multiple flights.") +
  theme(legend.position="bottom", plot.caption=element_text(hjust=0, size=12))
```
